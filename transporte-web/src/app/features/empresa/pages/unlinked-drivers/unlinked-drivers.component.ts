...
@Component({
  selector: 'app-unlinked-drivers',
  templateUrl: './unlinked-drivers.component.html',
  styleUrls: ['./unlinked-drivers.component.css']
})
export class UnlinkedDriversComponent implements OnInit {
  ...
  loadDrivers(pageIndex: number = 0, pageSize: number = 10) {
    ...
    this.driverService.getAvailableDrivers(pageIndex, pageSize,this.vehicleId,this.contextService.getTransportCompanyId()).subscribe({
  ...
  goToUnlinkedVehicles(): void {
    this.router.navigate(['/company/vehicles/linked-drivers',this.vehicleId]);
  }
  ...
  handleLinkDriverRow(row: any) {
      this.dialogService.openDialog(ModalWindowsTypes.INFO,
        ...
        ConfirmationMessages.LINK_DRIVER_TO_COMPANY_CONFIRMATION,
        {
          text: 'Vincular conductor',
          onClick: () => this.linkDriver(row)
        });
    }
    ...
    linkDriver(row: any) {
      ...
      this.driverService.affiliate(vehicleDriver).subscribe({
        next: (res) => {
